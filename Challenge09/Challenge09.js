...
const employeesBonus = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++); {
        if (arr.workHours > 8) {
            arr.number(arr.workHours) + 100;
        } else if (arr.workHours < 8) {
            arr.number(arr.workHours) + 50;
        }
    }
    arr = result;
    return arr;
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    for (let i = 0; i < mouseArray.length; i++) {
        for (let j = 0; j < keyBoardArray.length; j++) {
            if (mouseArray[i] + keyBoardArray[j] === 200) {
                return `price for your mouse is ${mouseArray[i]} price for your key Board Is ${keyBoardArray[j]}`;
            }
        }
    }
}
